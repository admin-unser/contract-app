    ...
    <div className="max-w-6xl mx-auto">
      ...
      {currentStep === 3 && (
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 bg-gray-50">
              <h2 className="text-base font-semibold text-gray-800">送信内容の確認</h2>
              <p className="text-sm text-gray-500 mt-0.5">以下の内容で署名依頼を送信します。</p>
            </div>
            <div className="p-5 space-y-5">
              {/* Document info */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">文書情報</h3>
                <div className="rounded-lg bg-gray-50 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-gray-800">{title || "無題"}</div>
                      <div className="text-xs text-gray-500">{file?.name ?? selectedTemplate?.name ?? ""}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Signers */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">署名者 ({createdSigners.length}名)</h3>
                <div className="space-y-2">
                  {createdSigners.map((s, i) => {
                    const color = SIGNER_COLORS[i % SIGNER_COLORS.length];
                    const signerFields = fields.filter((f) => f.signer_id === s.id);
                    return (
                      <div key={s.id} className="rounded-lg bg-gray-50 p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: color.hex }}>
                            {i + 1}
                          </div>
                          <div>
                            <div className="text-sm font-medium text-gray-800">{s.name || s.email}</div>
                            <div className="text-xs text-gray-500">{s.email}</div>
                            {s.company_name && <div className="text-xs text-gray-400">{s.company_name}</div>}
                          </div>
                        </div>
                        <span className="text-xs text-gray-500">署名欄: {signerFields.length}件</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {emailMessage && (
                <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">メール本文</h3>
